...
void loop() {
  ...
  if (escala1.is_ready()) {
    leitura1 = ((escala1.get_units(1) * fator_calibracao1) * gravidade) / area_sensor; // Retorna a leitura da variável escala com a unidade Newtons
  }
  if (escala2.is_ready()) {
    leitura2 = ((escala2.get_units(1) * fator_calibracao2) * gravidade) / area_sensor; // Retorna a leitura da variável escala com a unidade Newtons
  }
  if (escala3.is_ready()) {
    leitura3 = ((escala3.get_units(1) * fator_calibracao3) * gravidade) / area_sensor; // Retorna a leitura da variável escala com a unidade Newtons
  }
